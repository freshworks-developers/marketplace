...
- **Before `fdk validate`:** **`skills/fw-app-dev/SKILL.md`** → *Manifest + toolchain gate* — **`fw-setup`** if **FDK 10 + Node 24** missing; **`/fdk-migrate`** if not **Platform 3.0** or legacy **`engines`**; **never** downgrade toolchain or manifest to **FDK 9 / Node 18** instead of setup/migrate (except **LAST RESORT** in that file)
