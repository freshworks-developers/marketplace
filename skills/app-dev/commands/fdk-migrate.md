...
**Target:** **`platform-version` `"3.0"`**, **`modules`** layout, request templates, Crayons, and **`engines`** **`"node": "24.11.0"`**, **`"fdk": "10.0.1"`** (app-dev default).
...
2. Check FDK version:
   ...
   fdk version
   ```
   - **REQUIRED:** FDK **10.0.1** or newer **10.x** on Node 24.x (app-dev pins **`10.0.1`** in `engines` after migration)
