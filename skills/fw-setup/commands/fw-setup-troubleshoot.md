...
Run the **verbose** block from **`commands/fw-setup-status.md`**, then summarize:
...
append_nvm_loader() {
  ...
  echo "" >> "$rc"
  echo "# nvm (added by fw-setup-troubleshoot --fix $TS)" >> "$rc"
...
hint="# FDK 10.x / Node 24.11 (fw-setup-troubleshoot --fix $TS)"
for rc in "$HOME/.zshrc" "$HOME/.bashrc"; do
  [ -f "$rc" ] || continue
  grep -qF "fw-setup-troubleshoot" "$rc" 2>/dev/null && continue
  echo "" >> "$rc"
  echo "$hint" >> "$rc"
