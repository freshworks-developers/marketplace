...
## Windows Troubleshooting: "where fdk" shows nothing after downgrade

If `/fw-setup-downgrade` completed but `where fdk` returns empty in your terminal:

### 1. Use `where.exe fdk`, NOT `where fdk`

In PowerShell, **`where`** is an alias for **`Where-Object`**, not the Windows `where.exe` program.

```powershell
# ❌ WRONG (PowerShell alias — won't find fdk.exe)
where fdk
...
The downgrade script includes an automatic PATH refresh (line 278-281), but if you ran other commands between downgrade and checking `fdk`, manually refresh:
...
nvm-windows updates a system-wide symlink. Some terminal hosts cache the old symlink state. **Close and reopen** your PowerShell/Terminal window, then:

```powershell
node --version   # Should show v18.x (if downgraded to FDK 9.x) or v24.11.x (if downgraded within FDK 10.x)
where.exe fdk
fdk version
```

### 4. If still missing: verify npm global prefix
...
If `npm list` shows `@freshworks/fdk@9.x` (or `10.x`) but `where.exe fdk` is still empty, the Node `bin` folder may not be on PATH. Run:

```powershell
$nvmNodePath = (Get-Command node).Path | Split-Path -Parent
if ($env:Path -notlike "*$nvmNodePath*") {
  $env:Path = "$nvmNodePath;$env:Path"
}
where.exe fdk
```

### 5. Last resort: reinstall on the active Node
