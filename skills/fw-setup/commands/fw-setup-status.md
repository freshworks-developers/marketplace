---
name: fw-setup-status
description: Check FDK and Node.js installation status (/fw-setup status); optional --verbose diagnostics
always: true
...
**Closeout:** `/fw-setup-status` runs inline only. Do not spawn `fdk run`, tunnels, or watchers.

**Legacy alias:** **`/fw-setup-status`**.
