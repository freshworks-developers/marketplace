...
**Naming and copy:** Derive **`appName`** from manifest **`name`** or the app directory name; **`appDescription`** / **`appOverview`** from the user or manifest where available. **`supportEmail`** is required for **new** app submission — **ask the user** if unknown. **Do not** use **`git config user.email`** (may be missing or unsuitable for marketplace support).
