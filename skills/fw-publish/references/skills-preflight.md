...
| **Q2** | **New Marketplace listing or update existing listing?** | fw-publish only |
...
| **fw-publish** | **Step 2** — same manifest search + ask as app-dev | **Step 6** — ask new vs update; **`list_custom_apps`** + developer picks **`appId`** for updates (**not** `.fdk/app-info.json`) | same | **Step 3** before pack |
...
- **`fw-publish`** — explicit **Q2**/**Q3** at publish time; for **updates**, **`list_custom_apps`** + developer selects **`appId`** (**do not** route updates from `.fdk/app-info.json`). Optional **pre-publish confirm**.
