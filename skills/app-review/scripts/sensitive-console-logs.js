...
async function run(targetDir) {
  const files = await walkFiles(targetDir, ['.html', '.js', '.jsx', '.ts', '.tsx']);
  const details = [];

  for (const file of files) {
    for (const hit of collectMatches(file.content, SENSITIVE_LOG_PATTERN)) {
      details.push(
        createDetail(
          file.relativePath,
          'Sensitive values should never be logged to the browser console.',
          hit.line,
          hit.excerpt
        )
      );
  ...
  return details.length === 0
    ? createResult(true, 'No sensitive console logging patterns were detected.')
    : createResult(false, 'Sensitive console logging patterns were detected.', details);
}
