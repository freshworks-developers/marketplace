/**
 * Reads eval-results.json and writes eval-report.md + eval-report.html for PR attachment.
...
const mdRows = results.map(r => {
  const icon = r.pass ? '✅' : '❌';
  return `| \`${r.id}\` | ${r.skill} | ${r.label} | ${icon} | ${r.passed}/${r.total} |`;
});

const mdFailures = results
  .filter(r => !r.pass)
  .map(r => {
...
const markdown = `# Skill Eval Report

**Date:** ${date}
**Model:** \`${model}\`
**Result:** ${allPass ? '✅ All passed' : `❌ ${total - passed} failed`} (${passed}/${total})
...
${mdFailures.length > 0 ? `## Failures\n\n${mdFailures.join('\n')}` : '## All scenarios passed 🎉'}

---
*Generated by \`npm run eval\` — attach to PR for review.*
`;

await writeFile(join(__dirname, 'eval-report.md'), markdown, 'utf8');
...
<body>
  <div class="container">
    ...
    <footer>Generated by <code>npm run eval</code> — attach to PR for review.</footer>
  </div>
</body>
</html>
`;
...
console.log(`\nReports written (${passed}/${total} passed):`);
console.log(`  tests/eval-report.md`);
