...
|-------|---------|-----|----------------|
| **1 — Static** | `cd tests && npm test` | Yes | Skill file structure, gates, scripts, templates (~145 tests) |
...
Publish-guard E2E uses `lib/llm-log-publish-actions.py` to detect **executed** publish steps (shell `upload-app.sh` / `fdk pack`, MCP upload/submit) — not prose or read/grep tool calls. Unit tests: `e2e-publish-guard-parser.test.mjs`.

**Expected:** 145 pass, 0 fail. Implementation: `skill-static.test.js`.
...
**68 scenarios** across all 5 skills + orchestration spec. Source of truth: `skill-eval.test.js` (`SCENARIOS` array). Do not duplicate the scenario list in this doc — open the report or source file for IDs and assertions.
