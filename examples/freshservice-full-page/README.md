...
**Instance methods (demo):** the full page includes **`client.instance.context()`** (prints JSON into the page) and **`client.instance.resize({ height: "480px" })`** with **try/catch**, matching the patterns in **`skills/fw-app-dev/references/api/instance-method-docs.md`** (max height **700px** for instances).

## Prerequisites

- **FDK 10.x** and **Node.js 24.x** (repo `skills/fw-setup` if the CLI is missing).
